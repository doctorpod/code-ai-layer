...
# Patterns are loaded from _AI/local/scripts/sensitive-patterns.conf.
...
PATTERNS_FILE="_AI/local/scripts/sensitive-patterns.conf"
...
if [ "$FOUND" -eq 1 ]; then
    ...
    echo "To update the pattern list, edit _AI/local/scripts/sensitive-patterns.conf."
    exit 1
fi
